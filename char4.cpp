...
struct Monster
{
    int x,y=Ground2,dx=-8,monsterindex;
    bool alive;
} monster[10];
...
int bg2upperindex=0;
int bg2index=0;
char idle[][100]= {"Char/IDLE/idle-up - Copy (1).bmp",
                   "Char/IDLE/idle-up - Copy (2).bmp",
                   "Char/IDLE/idle-up - Copy (3).bmp",
                   "Char/IDLE/idle-up - Copy (4).bmp",
                   "Char/IDLE/idle-up - Copy (5).bmp",
                   "Char/IDLE/idle-up - Copy (6).bmp",
                   "Char/IDLE/idle-up - Copy (7).bmp",
                   "Char/IDLE/idle-up - Copy (8).bmp",
                   "Char/IDLE/idle-up - Copy (9).bmp",

                  };
int idleindex=0;
char jump[][100]= {"Char/JUMP/jump_(11).bmp",
                   "Char/JUMP/jump_(12).bmp",
                   "Char/JUMP/jump_(13).bmp",
                   "Char/JUMP/jump_(14).bmp",
                   "Char/JUMP/jump_(15).bmp",
                   "Char/JUMP/jump_(16).bmp",
                   "Char/JUMP/jump_(17).bmp",
                   "Char/JUMP/jump_(18).bmp",
                   "Char/JUMP/jump_(19).bmp",
                   "Char/JUMP/jump_(20).bmp"
                  };
int jumpindex=0;
char run[][100]= {"Char/RUN/Run_(11).bmp",
                  "Char/RUN/Run_(12).bmp",
                  "Char/RUN/Run_(13).bmp",
                  "Char/RUN/Run_(14).bmp",
                  "Char/RUN/Run_(15).bmp",
                  "Char/RUN/Run_(16).bmp",
                  "Char/RUN/Run_(17).bmp",
                  "Char/RUN/Run_(18).bmp"
                 };
int runindex=0;
...
void iDraw()
{
    ...
    if(scene==0)
    {
        ...
        {
            iShowBMP2(press[i].x,press[i].y,"playbutton.bmp",0xffffff);
        }
    }
    if(scene==1)
    {
    ...
    if(scene==2)
    {
        ...
        for(int i=0; i<birdnum; i++)
        {
            ...
            /*else if(!bird[i].bmbmove && bird[i].explosioncounter<=10){
                ++bird[i].explosioncounter;
                iShowBMP2(bird[i].bmbx,bird[i].bmby,"Char/SHOOT/explosion1.bmp",0xffffff);
            }*/
            //bird[i].explosioncounter=bird[i].explosioncounter>10?0:bird[i].explosioncounter;
        }
    ...
    if(key=='j' && mc.x<WindowWidth)
    {
        if(scene==2)
        {
            ...
            for(int i=0; i<birdnum; i++)
            {
                ...
                if(bird[i].bmbmove) bird[i].bmbx-=relativespeed;
            }
    ...
    if(scene==2)
    {
        ...
        if(monstertemp>5)
        {
            for(int i=0; i<monstercount; i++)
            {
                ...
                if(monster[i].alive && (monster[i].x-mc.x<100 || monster[i].x<5))
                {
                    monster[i].alive=false;
                }
                if(!monster[i].alive)
                {
                    monster[i].alive=random(1,100)>70?true:false;
                    if(monster[i].alive)
                    {
    ...
    if(scene==2)
    {
        for(int i=0; i<birdnum; i++)
        {
            if(!bird[i].alive)
            {
                ...
                bird[i].bmbx=random(300,800);
                bird[i].bmby=bird[i].y;
                bird[i].bmbmove=false;
            }
            if(bird[i].alive)
            {
                bird[i].x+=bird[i].dx;
            ...
            if(!bird[i].bmbmove && bird[i].x<bird[i].bmbx && bird[i].bmby!=-1)
            {
                bird[i].bmbmove=true;
            }
            if(bird[i].bmbmove && bird[i].bmby>Ground2)
            {
                bird[i].bmby-=5;
            }
            else if(bird[i].bmbmove && bird[i].bmby<=Ground2)
            {
                bird[i].bmbmove=false;
                bird[i].bmby=-1;
            }
            if(bird[i].x<10 && !bird[i].bmbmove)
            {
                bird[i].alive=false;
